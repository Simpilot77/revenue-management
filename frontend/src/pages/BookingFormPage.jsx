...
import { buildInvoicePreviewData, exportInvoiceFromData } from '../utils/pdfExport';
...
function InvoicePreviewModal({ data, onClose, onLangChange, onChange }) {
  ...
  const updateExtraItem = (i, field, value) => {
    const updated = extraItems.map((item, idx) => idx === i ? { ...item, [field]: value } : item);
                ...
                {extraItems.map((item, i) => (
                  <div key={i} className="border border-gray-200 rounded-xl p-3 bg-gray-50 space-y-2">
                    ...
                    <div className="text-right text-xs text-gray-500 font-medium">
                      Gesamt: {new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format((item.qty || 0) * (item.unit_price || 0))}
                    </div>
                  </div>
                ))}
              </div>
