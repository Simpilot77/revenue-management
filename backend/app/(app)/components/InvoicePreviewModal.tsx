...
import { exportInvoiceFromData, previewInvoiceFromData } from '../tasks/pdfExport'

// ─── Utilities ────────────────────────────────────────────────────────────────

function splitInvoiceNumber(invoiceNumber: string) {
  const parts = String(invoiceNumber || '').split('-')
  if (parts.length >= 3) return { prefix: parts[0], year: parts[1], suffix: parts.slice(2).join('-') }
  return { prefix: '', year: '', suffix: invoiceNumber || '' }
}

function composeInvoiceNumber({ prefix, year, suffix }: { prefix: string; year: string; suffix: string }) {
  ...
  return (
    <div className="flex items-center gap-2 mb-2 flex-wrap">
      ...
      >
        ...
        {presets.map((p: any, i: number) => <option key={i} value={String(i)}>{p.name}</option>)}
      </select>
      <button
        type="button"
        disabled={sel === ''}
...
export default function InvoicePreviewModal({ data, onClose, onLangChange, onChange, onRecorded, settings }: Props) {
  ...
  const handleAutoInvoiceNumber = async () => {
    ...
    setAutoLoading(true)
    try {
      const res = await fetch(`/api/bookings/next-invoice-number?house_id=${data._house_id}`)
      const json = await res.json()
      const next = json.invoice_number
  ...
  const handleAutoCustomerNumber = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const json = await res.json()
        set('customer_number', json.customer_number || '')
      }
    } finally { setAutoLoading(false) }
  }
  ...
  const handlePreview = async () => {
    ...
    setGenerateError('')
    try {
      const url = await previewInvoiceFromData(data)
  ...
  const F = ({ label, field, type = 'text', rows = 2, colSpan, readOnly }: any) => (
    <div className={colSpan === 2 ? 'md:col-span-2' : colSpan === 3 ? 'col-span-full' : ''}>
      ...
      {type === 'textarea' ? (
        <textarea className={inp} rows={rows}
          value={data[field] || ''} onChange={e => set(field, e.target.value)} />
      ) : (
        <input type={type} className={`${inp}${readOnly ? ' bg-gray-50 text-gray-400' : ''}`}
          readOnly={readOnly}
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white rounded-xl shadow-2xl w-full" style={{ maxWidth: 780 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="md:col-span-2">
                ...
                {data.billing_address_freetext && (
                  <p className="text-xs text-blue-600 mt-1">✓ Freitext aktiv – Adressfelder oben werden ignoriert.</p>
                )}
              </div>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="md:col-span-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <div style={{ width: 90 }}>
                    ...
                    <input className={`${inp} font-mono`} value={invParts.prefix} placeholder="15a"
                  ...
                  <div style={{ width: 80 }}>
                    ...
                    <input className={`${inp} font-mono text-center`} value={invParts.year} placeholder="2026" maxLength={4}
                  ...
                  <div style={{ width: 110 }}>
                    ...
                    <input className={`${inp} font-mono text-center`} value={invParts.suffix} placeholder="1001" maxLength={4}
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  <input className={`${inp} flex-1`} value={data.service_period || ''}
                    onChange={e => set('service_period', e.target.value)}
                    placeholder="01.01.2026 – 08.01.2026" />
                  <button type="button"
                    ...
                    onClick={() => setShowDatePicker(v => !v)}>📅</button>
                </div>
                {showDatePicker && (
                  <div className="mt-2 p-3 border border-blue-200 rounded-xl bg-blue-50/60 space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input type="date" className={inp} value={periodDates.from}
                          onChange={e => { const next = { ...periodDates, from: e.target.value }; setPeriodDates(next); applyDateRange(e.target.value, next.to) }} />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">Bis</label>
                        <input type="date" className={inp} value={periodDates.to}
                          onChange={e => { const next = { ...periodDates, to: e.target.value }; setPeriodDates(next); applyDateRange(next.from, e.target.value) }} />
                      </div>
                    </div>
                    <button type="button"
                      className="w-full text-xs py-1.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700"
                      onClick={() => setShowDatePicker(false)}>✓ Schließen</button>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" disabled={autoLoading}
                    className="text-xs px-2.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg border border-blue-200 font-medium disabled:opacity-40"
                    onClick={handleAutoCustomerNumber}>🔄</button>
                </div>
              </div>
              <F label="Ansprechpartner" field="contact_person" />
              {data.external_reference && (
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">Externe Buchungsnr.</label>
                  <input readOnly className={`${inp} bg-gray-50 text-gray-500 font-mono`} value={data.external_reference} />
                </div>
              )}
            </div>
          </section>
              ...
              <div className="border border-gray-200 rounded-xl p-3 bg-gray-50 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <div className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-600 font-mono font-semibold">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <button type="button" onClick={addExtraItem}
                className="text-xs px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg font-medium border border-blue-200">
                + Position hinzufügen
              </button>
            </div>
            {extraItems.length === 0 ? (
                  ...
                  <div key={i} className="border border-gray-200 rounded-xl p-3 bg-gray-50">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      ...
                      <div>
                        ...
                        <input type="number" min="0" step="0.5" className={inp} value={item.qty}
                      ...
                      <div>
                        ...
                        <input type="number" min="0" step="0.01" className={inp} value={item.unit_price}
                          onChange={e => updateExtraItem(i, 'unit_price', parseFloat(e.target.value) || 0)} />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">Gesamt</label>
                        <div className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-100 font-mono font-semibold text-gray-600">
                          {fmtEur((item.qty || 0) * (item.unit_price || 0))}
                        </div>
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-xs font-medium text-gray-500 mb-1">Vermerk</label>
                        <input className={inp} value={item.note}
                          onChange={e => updateExtraItem(i, 'note', e.target.value)} />
                      </div>
                      <div className="flex items-end">
                        <button type="button" onClick={() => removeExtraItem(i)}
                          className="w-full text-xs px-2 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg font-medium border border-red-200">
                          🗑 Entfernen
                        </button>
                      </div>
                    </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input type="number" step="0.01" className={`${inp} font-mono`}
              ...
              <div>
                ...
                <input readOnly className={`${inp} font-mono bg-gray-50 text-gray-400`}
                  value={fmtEur(data.vat_amount)} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Brutto-Gesamt (€)</label>
                <input type="number" step="0.01" className={`${inp} font-mono font-semibold`}
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-xl space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button type="button"
                className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-5 py-2 rounded-lg flex items-center gap-2 disabled:opacity-50"
